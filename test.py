...
#Loading the model
	def load_model(self,model):
	        self.mod_bin = model
		self.mod_xml = '.'+self.mod_bin.spit('.')+'(.xml)
	    	self.plugin = IECore()	
		self.network = IENetwork(model=self.mod_xml, weights=self.mod_bin)
	        self.exec_network = self.plugin.load_network(self.network,device_name='CPU')
...
def main(args):
    ...
    try:
        ...
        while cap.isOpened():
            ret, frame=cap.read()
	    frame = pd.preprocess_input(frame)
